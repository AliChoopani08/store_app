...
public class ControllerSearchProduct {
    ...
    public ResponseEntity<ApiResponse<Page<ProductSummary>>> searchDynamicProduct(@ModelAttribute SearchProductRequest searchProductRequest,
                                                                                  @PageableDefault(size = 20, sort = "name", direction = DESC)
                                                                                   @ParameterObject Pageable pageable) {

        final Page<ProductSummary> productResponses = service.searchProducts(searchProductRequest, pageable);
